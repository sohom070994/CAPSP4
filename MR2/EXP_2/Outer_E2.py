...
for i in range(0,len(shrtspth)-1):
    for j in range(shrtspth[i+1]-shrtspth[i]):
        propsr.append(sum((x < (shrtspth[i+1]-1)*40 and x >= (shrtspth[i]-1)*40) for x in sr)/(40*(shrtspth[i+1]-shrtspth[i])))
        propsc.append(sum((x < (shrtspth[i+1]-1)*40 and x >= (shrtspth[i]-1)*40) for x in sc)/(40*(shrtspth[i+1]-shrtspth[i])))
