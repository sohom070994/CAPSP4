...
def weightscs(n1, n2):
    ...
    wtd=(wtcalc(ar,sr,n1,n2,U_triage)/wtr)+(wtcalc(ac,sc,n1,n2,U_crisis)/wtc)
...
s = in_pr_2s(pr, pc, ar, ac, t_graph, c_graph, "seq")
sr = s[0]
sc = s[1]
...
edges, graph = create_graph(nodes, num_changes, 2, 1, ar, sr, ac, sc)
paths, lengths= shortestPath(graph, u-1, v-1,num_changes+1)
...
policy.to_csv(f'PolicySeq_E2_{pr[0]}_{pc[0]}.csv', index = False)
