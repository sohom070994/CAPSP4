...
import math
#import SPfuncs
from InnerProblem_create_graph_3 import in_pr_2s
import pandas as pd
#%%
def memoize(fn):
    cache = {}
...
def weightscs(n1, n2):
    global wtr, wtc, wtp, ar, sr, ac, sc, ap, sp, U_triage, U_crisis, U_psych
    
    wtd=(wtcalc(ar,sr,n1,n2,U_triage)/wtr)+(wtcalc(ac,sc,n1,n2,U_crisis)/wtc)+ (wtcalc(ap,sp,n1,n2,U_psych)/wtp)
...
graph.
'''
def create_graph(nodes, num_changes, ns, s, ar, sr, ac, sc, ap, sp):
    global U_triage, U_crisis, U_psych
    ...
    for i in range(1,len(nodes)):
        for j in range(i+1,(len(nodes)+1)):
            if ns == 1:
                ...
                if s ==3:
                    edges.append([i, j, weightsrs(ap, sp, i, j, U_psych)])
            if ns == 3:
...
U_psych = math.floor(slots*pp[0]/100)

t_graph = g_triage
c_graph = g_crisis
p_graph = g_psych

s = in_pr_2s(pr, pc, pp, ar, ac, ap, t_graph, c_graph, p_graph, "BnB")
sr = s[0]
sc = s[1]
sp = s[2]
...
s1 = in_pr_2s(pr, pc, pp, ar, ac, ap, t_graph, c_graph, p_graph, "NoBnB")
s1r = s1[0]
s1c = s1[1]
s1p = s1[2]

edges, graph = create_graph(nodes, num_changes, 1, 1, ar, s1r, ac, s1c, ap, s1p)
paths, lengths= shortestPath(graph, u-1, v-1,num_changes+1)
wtr = lengths[lengths.index(min(lengths))]
edges, graph = create_graph(nodes, num_changes, 1, 2, ar, s1r, ac, s1c, ap, s1p)
paths, lengths= shortestPath(graph, u-1, v-1,num_changes+1)
wtc = lengths[lengths.index(min(lengths))]
edges, graph = create_graph(nodes, num_changes, 1, 3, ar, s1r, ac, s1c, ap, s1p)
paths, lengths= shortestPath(graph, u-1, v-1,num_changes+1)
wtc = lengths[lengths.index(min(lengths))]
...
edges, graph = create_graph(nodes, num_changes, 3, 1, ar, sr, ac, sc, ap, sp)
paths, lengths= shortestPath(graph, u-1, v-1,num_changes+1)
...
shrtspth = paths[lengths.index(min(lengths))]
proptbl = np.zeros((18,4))
propsr = []
propsc = []
propsp = []
count = 0
for i in range(0,len(shrtspth)-1):
    for j in range(shrtspth[i+1]-shrtspth[i]):
        propsr.append(sum((x < (shrtspth[i+1]-1)*slots_per_week and x >= (shrtspth[i]-1)*slots_per_week) for x in sr)/(slots_per_week*(shrtspth[i+1]-shrtspth[i])))
        propsc.append(sum((x < (shrtspth[i+1]-1)*slots_per_week and x >= (shrtspth[i]-1)*slots_per_week) for x in sc)/(slots_per_week*(shrtspth[i+1]-shrtspth[i])))
        propsp.append(sum((x < (shrtspth[i+1]-1)*slots_per_week and x >= (shrtspth[i]-1)*slots_per_week) for x in sp)/(slots_per_week*(shrtspth[i+1]-shrtspth[i])))
        proptbl[count][0] = count+1
        proptbl[count][1] = propsr[count]
        proptbl[count][2] = propsc[count]
        proptbl[count][3] = propsp[count]
        count += 1


#%%
policy = pd.DataFrame(proptbl, columns = ['Week', 'Triage', 'Crisis','Psych'])
policy.to_csv(f'Policy3s_E2_{pr[0]}_{pc[0]}_{pp[0]}.csv', index = False)
