...
for i in range(0,len(shrtspthr)-1):
    for j in range(shrtspthr[i+1]-shrtspthr[i]):
        propsr.append(sum((x < (shrtspthr[i+1]-1)*40 and x >= (shrtspthr[i]-1)*40) for x in sr)/(40*(shrtspthr[i+1]-shrtspthr[i])))
...
for i in range(0,len(shrtspthc)-1):
    for j in range(shrtspthc[i+1]-shrtspthc[i]):
        propsc.append(sum((x < (shrtspthc[i+1]-1)*40 and x >= (shrtspthc[i]-1)*40) for x in sc)/(40*(shrtspthc[i+1]-shrtspthc[i])))
        proptbl[count][0] = count+1
...
#policy.to_csv(f'Policy_E1_Sohom_{pr[0]}_{pc[0]}.csv', index = False)
